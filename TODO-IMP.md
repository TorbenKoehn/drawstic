# TODO-IMP — Improvements from the Scene-DX Evaluation report
...
decisions need an ADR first (see `TODO.md` ground rule). **Every task that changes
...
- Proven, copyable recipe snippets for the motifs every scene reinvented: palm/tree,
  ...
  starfield/scatter. Keep std abstract (TODO.md §5.1 policy) — this is docs, not std.
...
### 3.1 Fix `--ascii` ramp (extends TODO.md §9.1) — ✅ done
...
- Files: `src/preview.ts`, `src/inspect.ts`, tests; TODO.md §9.1 supersedes/merges.
