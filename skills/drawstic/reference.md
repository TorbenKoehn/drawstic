...
| `drawstic critique <file> [--as icon\|scene\|character\|item] [--strict] [--json]` | pixel-based, vision-free quality checks (`C0xx`) over every rendered non-parametric drawing. Category-agnostic (always run): C001 empty/near-empty, C003 optical centering, C004 value/contrast spread, C006 palette/complexity budget, C008 interior pinholes, C012 transparent trailing edge row. `--as` selects a category profile (thresholds, never inferred) and opts in the pixel-geometry checks — **C007** floating-part/seam (8-connected components + chamfer distance; `character` only) and **C005** stroke width (chamfer distance; `icon`/`item`/`character`); without `--as` only the agnostic subset runs plus a `C000` info nudge. All findings default to `warning` (exit 0, never blocking); `--strict` promotes the must-fix subset (C001/C007/C008/C012, plus C003 for `icon`/`item`) to `error` (exit 1) — the CI regression gate. Each finding carries `{measured, threshold, fix}`. JSON: `{diagnostics, critique: {pass, profile, strict, failedCodes, drawings:[{name,width,height,bbox,coveredPixelCount,opaquePixelCount,distinctColorCount,unknownColorCount,luminance,componentCount?,minStrokeWidth?,checks:[…]}]}}` — the metric bundle is a superset of `render --inspect`, readable even at zero findings. |
