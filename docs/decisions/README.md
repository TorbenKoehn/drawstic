...
| [0095](0095-skeleton-and-pose.md) | Skeleton/pose rig: `skeleton NAME:` joints (anchored/FK, angle limits) + deterministic forward kinematics, `pose NAME over SKELETON:` angle deltas + view, `fit part.pin bone JOINT` (inherits orientation), auto-Z from bone depth, `render --explain` solved rig | Accepted (refines 0087, 0092, 0093) |
| [0096](0096-language-freeze-for-1-0.md) | Language freeze for 1.0: `import`→`image`/`band`→`ribbon`/`fit…shadow`→`ground`/`ambientOcclusion`→`ao`/`pal`→`palette`/`grad`→`gradient` renames; `cap`/`join`/`castShadow`/`seed N`/`grayscale`/`drawstic N`/bare-int size/bare filter statement removed; `tileset`+`atlas` merged into one `atlas` (optional `tile WxH`); third light-resolution tier (module's sole `light` binding); uniform builtin reservation; recipe-relative export paths; `mix`'s colour-space arg as a bare keyword | Accepted (refines 0094, 0088, 0086, 0087, 0065) |
| [0097](0097-canonical-shading-floor.md) | Canonical shading floor: remove the raw hand-light quartet (`rim`/`shadeRegion`/`lightRegion`/`ambientOcclusion`), add `Region.edge()` | Accepted (refines 0096 §8, 0086, 0094) |
| [0098](0098-multi-target-export-blocks.md) | Multi-target `export` blocks: comma-separated target list, omitted path defaults to the drawing name, `dir` (bareword) + `file` stem template with six template-only inflectors; `{ext}`/`{full}`/`title`/`plural`/dates rejected; `E028` + `W019`; cross-export artifact collision lifted to `check` | Accepted (refines 0096 §6, 0019, 0006, 0030) |
| [0099](0099-opt-in-filtered-stamp-resampling.md) | Opt-in filtered stamp resampling: bare `aa` flag on `stamp`/`fit`, 4×4 area sampling on ADR-0040's pinned grid, integer premultiplied gamma-sRGB accumulation, byte-identical to NN on mirrors/`rot180`/integer scale+shift (and on quarter-turns only at equal w/h parity — §3 amended 2026-07-27); `W018` no-op lint | Accepted (lands 0043 §5; refines 0044, 0040, 0025, 0013) |
| [0100](0100-model-comparison-eval-corpus.md) | A model-comparison eval corpus, kept out of `examples/`: `evals/model-comparison/<model>/<category>.drw`, frozen once rendered, gated for validity (parses/`check`-clean/declared size) not craft | Accepted |
